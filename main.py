...
def init_db():
    ...
    existing_count = db.query(databasemodels.Product).count()
...
def get_product_by_id(product_id: int, db: Session = Depends(get_db)):
    product = db.query(databasemodels.Product).filter(databasemodels.Product.id == product_id).first()
    if product:
        return product
...
def update_product(product_id: int, product: Product, db: Session = Depends(get_db)):
    db_product = db.query(databasemodels.Product).filter(databasemodels.Product.id == product_id).first()
    if not db_product:
